...
public class JettyLauncher extends ServletContainerLauncher {
   /**
    * Ant compiler adapter for Eclipse Java compiler, but with default target
    * and source compatibility set to Java 6.
    */
   ...
   protected static final class WebAppContextWithReload extends WebAppContext {
      /**
       * Specialized {@link WebAppClassLoader} that allows outside resources to
       * be brought in dynamically from the system path. A warning is issued
       * when this occurs.
       */
      private class WebAppClassLoaderExtension extends WebAppClassLoader {
         private static final String META_INF_SERVICES = "META-INF/services/";
         private final ClasspathPattern systemClassesFromWebappFirst = new ClasspathPattern (new String [] { "-javax.servlet.", "javax.", });
         private final ClasspathPattern allowedFromSystemClassLoader = new ClasspathPattern (new String [] { "org.eclipse.jetty.",
               // Jasper
         ...
         public URL findResource (String name) {
            ...
            // to add gwt-user when using a "Developer SDK" in Eclipse.
                  "log4j.xml".equals (name) || "log4j.properties".equals (name)) {
               return found;
            }
      ...
      protected void doStop () throws Exception {
         ...
         Class <?> jdbcUnloader = getClassLoader ().loadClass ("com.google.gwt.dev.shell.jetty.JDBCUnloader");
   ...
   private void jreLeakPrevention (TreeLogger logger) {
       ...
       * Token poller thread with TCCL equal to the web application class
       * loader.
       * 
       ...
       * app is the first to make the calls below the web application class
       * loader will be pinned in memory.
       */
